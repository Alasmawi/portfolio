// Sourced from React Bits (reactbits.dev), MIT + Commons Clause license —
// https://github.com/DavidHDev/react-bits/tree/main/src/content/Backgrounds/Threads
// Real WebGL (via ogl), not a canvas/CSS approximation: ~40 flowing lines
// driven by per-pixel Perlin noise in the fragment shader, with optional
// mouse-interaction easing. Pauses via IntersectionObserver + document
// visibility so it never burns GPU when off-screen or backgrounded, and
// caps internal render resolution so large/high-DPI screens stay smooth.
...
const Threads = ({ color = [1, 1, 1], amplitude = 1, distance = 0, enableMouseInteraction = false, ...rest }) => {
  ...
  useEffect(() => {
    ...
    // Touch mirrors the mouse behaviour — without this, the interactive
    // variant would silently do nothing on the mobile devices most people
    // actually view a portfolio on.
